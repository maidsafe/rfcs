...
the direction the network is evolving in.
...
changes to SAFE libraries, dependencies, algorithms or the RFC process itself. What constitutes
a "substantial" change is evolving based on community norms, but may include
the following.

   - Any semantic or syntactic change to the existing algorithms and process that is not a bugfix.
   - Any proposed additions to exsiting algorithms
...
filing and discusssing ideas
on the [RFC issue tracker][issues], and occasionally posting
'pre-RFCs' on [discussion forum][discuss] for early
review.
...
[discuss]: http://forum.safenetwork.io/c/development
...
to the constributor team for a final decision.
...
The core team (includes pods)  makes final decisions about RFCs after 
...
implementation in the library affected repository; thus that associated 
issue can be assigned a priority via the triage process that the team 
uses for all issues in the appropriate repository.
